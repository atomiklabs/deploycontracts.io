...
> {
  ...
  async connect({ chainId }: { chainId?: number } = {}) {
    try {
      ...
      if (this.options?.shimDisconnect)
        storage?.setItem(shimDisconnectKey, true)
  ...
  async disconnect() {
    ...
    if (this.options?.shimDisconnect)
      storage?.removeItem(shimDisconnectKey)
  }
  ...
  async getProvider() {
    if (typeof window !== 'undefined' && !!window.cosmos)
      this.#provider = window.cosmos
  ...
  async isAuthorized() {
    try {
      if (
        ...
        !storage?.getItem(shimDisconnectKey)
      )
        return false
